...
from PyQt5.QtWidgets import QApplication, QMainWindow, QFileDialog, QWidget, QInputDialog, \
    QGraphicsDropShadowEffect, QMessageBox, QListWidgetItem, QSlider
from Main_styles import *


# Виджет окна с выбором плейлистов
...
class Main(QMainWindow):
    def __init__(self):
        ...
        self.setup_timer()  # Установка таймера для слайдера
    ...
    def duration_changed(self):  # Установка максимального значения слайдера
        self.slider.setMaximum(self.mediaPlayer.duration())

    def setup_slider(self):  # Настройка слайдера
        self.slider.setValue(0)
        self.slider: QSlider
        self.slider.sliderPressed.connect(self.pressed)

    # Осуществление перемотки трека
    def pressed(self):
    ...
    def update_slider(self):  # Обновление(движение) слайдера
    ...
    def apply_styles(self):
        ...
        # SLIDER
        self.slider.setStyleSheet(DURATION_SLIDER_STYLES)
    ...
    def add_new_track(self):
        ...
        if self.cur_playlist == "Все треки":  # Добавление нового трека в БД
            ...
            try:
                with open(fname, "rb") as track_bytes:
                    ...
                    if fname.endswith(".mp3"):
                        if "-" in fname:
                            ...
                            track_name = fname.split("/")[-1].split('.mp3')[0]
                    elif fname.endswith(".wav"):
                        if "-" in fname:
                            artist_name = fname.split("/")[-1].split('.wav')[0].split("-")[0]
                            track_name = fname.split("/")[-1].split('.wav')[0].split("-")[1]
                        else:
                            track_name = fname.split("/")[-1].split('.wav')[0]
                ...
                QMessageBox.warning(self, "Внимание!", "Такого файла не существует!")
        else:
    ...
    def transform_playlist_name(self, playlist_name):
        tmp_name = ""
        capitalized = 0
        for letter in playlist_name:
            if letter.isalpha():
                tmp_name += letter if capitalized else letter.upper()
                capitalized = 1
            else:
                capitalized = 0
    ...
    def create_playlist(self):
        ...
        if ok_pressed:
            # Преобразование названия плейлиста
            playlist_name = self.transform_playlist_name(playlist_name)
        ...
        if event.key() == Qt.Key_Delete and self.menu_isopen:
            if self.menu.playlists.currentItem().text() in ["Все треки", "Понравившиеся"]:
                ...
                if ok_pressed == QMessageBox.Yes:
                    ...
                    # Удаление таблицы плейлиста из БД
